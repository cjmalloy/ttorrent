...
public class Client extends Observable implements Runnable,
	...
	public synchronized void info() {
		...
		logger.info(
				"BitTorrent client {}, {}/{}/{} peers, {}/{}/{} pieces "
						+ "({}%), {}/{} kB/s.",
				new Object[] { this.getState().name(), choked,
	...
	private SharingPeer getOrCreatePeer(byte[] peerId, String ip, int port) {
		...
		synchronized (this.peers) {
			peer = this.peers.get(search.hasPeerId() ? search.getHexPeerId()
			...
			this.peers.putIfAbsent(peer.hasPeerId() ? peer.getHexPeerId()
	 ...
	 * 
	 * @param answer
	 *            The B-decoded answer map.
	 * @see <a
	 *      href="http://wiki.theory.org/BitTorrentSpecification#Tracker_Response">BitTorrent
	 *      tracker response specification</a>
	 */
	@Override
		...
		synchronized (peer) {
			...
			if (!peer.isBound()
				...
				if (!this.service.connect(peer)) {
					...
					this.peers.remove(peer.hasPeerId() ? peer.getHexPeerId()
	 ...
	 * 
	 * @param s
	 *            The connected socket to the remote peer. Note that if the peer
	 *            somehow rejected our handshake reply, this socket might very
	 *            soon get closed, but this is handled down the road.
	 * @param peerId
	...
	public void handleNewPeerConnection(Socket s, byte[] peerId) {
		SharingPeer peer = this.getOrCreatePeer(peerId, s.getInetAddress()
	...
	public static void main(String[] args) {
		...
		try {
			Client c = new Client(InetAddress.getByName(System
					.getenv("HOSTNAME")), SharedTorrent.fromFile(new File(
					args[0]), new File(args.length > 1 ? args[1] : "/tmp")));
			c.share();
