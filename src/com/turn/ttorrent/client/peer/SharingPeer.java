...
public class SharingPeer extends Peer implements MessageListener {
	 ...
	 * 
	 * @param message
	 *            The message to send to the remote peer throh our peer
	 *            exchange.
	 */
	...
	private synchronized void requestNextBlocks() {
		...
		while (this.requests.remainingCapacity() > 0
			...
			Message.RequestMessage request = Message.RequestMessage.craft(
					this.requestedPiece.getIndex(), this.lastRequestedOffset,
	...
	private synchronized Set<Message.RequestMessage> cancelPendingRequests() {
		Set<Message.RequestMessage> requests = new HashSet<Message.RequestMessage>();
	...
	public synchronized void handleMessage(Message msg) {
		...
		case REQUEST:
			...
			if (request.getLength() > Message.RequestMessage.MAX_REQUEST_SIZE) {
		...
		case PIECE:
			...
			try {
				...
				if (piece.getOffset() + piece.getBlock().capacity() == p.size()) {
					p.validate();
	...
	public static class DLRateComparator implements Comparator<SharingPeer> {
		...
		public int compare(SharingPeer a, SharingPeer b) {
			return new Rate.RateComparator().compare(a.getDLRate(),
					b.getDLRate());
		}
	}
	...
	public static class ULRateComparator implements Comparator<SharingPeer> {
		@Override
		public int compare(SharingPeer a, SharingPeer b) {
			return new Rate.RateComparator().compare(a.getULRate(),
					b.getULRate());
		}
	}
	...
	public String toString() {
		return new StringBuilder(super.toString()).append(" [")
				...
				.append((this.interesting ? "I" : "i")).append("]").toString();
	}
}
