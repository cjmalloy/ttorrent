...
class PeerExchange {
	 ...
	 * @param socket
	 *            The connected socket to the peer.
	 */
	...
	public void close() {
		...
		if (!this.socket.isClosed()) {
			try {
				...
				// But join the outgoing thread to let it finish serving the
				// queue.
				this.out.join();
	...
	private class IncomingThread extends Thread {
		...
		public void run() {
			try {
				...
				if (Client.getInputStreamWrapperClass() != null) {
					try {
						is = new DataInputStream((InputStream) Client
								.getInputStreamWrapperClass()
	...
	private class OutgoingThread extends Thread {
		...
		public void run() {
			try {
				...
				logger.trace("Could not send message to {}: {}", new Object[] {
						peer, ioe.getMessage(), ioe });
				peer.unbind(true);
			}
		}
	}
